...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.jobmaster.JobMasterEditForm" />
...
<% 
	if (BuildConfig.DMODE)
		System.out.println("Job Master Edit");
	int strDefault = 0;
	String radioChecked="";
	...
	try
   {
		...
		if (BuildConfig.DMODE)
			System.out.println("frm.getTrGenId(): "+frm.getTrGenId());
		if (!frm.getTrGenId().equalsIgnoreCase(""))
		{
			...
			if (BuildConfig.DMODE)
				System.out.println("Vector JobDetails: "+vecJbDet);
		}
		/* Listing all the JobOperations under a particular JobName */
		if (BuildConfig.DMODE)
			System.out.println("Integer.parseInt(frm.getTrJobId()): "+Integer.parseInt(frm.getTrJobId()));
		...
		if (BuildConfig.DMODE)
			System.out.println("vecTrJbDet(Size): "+vecTrJbDet.size()+" & "+vecTrJbDet);
...
var flag = 0;
	...
	function addRow(formObj)
	{
		...
		obj.children(a-1).children(0).children(4).children(0).checked = false;
	}
	...
	function trCustName()
	{
		...
		document.forms[0].trGenId.value = "";
		document.forms[0].focusPoint.value = 2;
		document.forms[0].submit();
	}
	...
	function transOperations(formObj)
	{
		var temp = document.forms[0];
		if (temp.checkValue != undefined)
		{
			...
			{
				alert ("No Operation to Transfer!");
				return false;
			}

			var obj1 = document.getElementById("tbl");
			var newNode = obj1.children(0).cloneNode(true);
			var tab = obj1.children(0);
			...
			/* Table values of OperGrpName and OperName */
			var gName = new Array();
			var oName = new Array();
			var count = obj1.children.length;
			var t = 0;
			for (var i = 0; i < count; i++)
			{
				gName[i] = obj1.children(i).children(0).children(2).children(0).value;
				oName[i] = obj1.children(i).children(0).children(3).children(0).value;
			}

			/* Fetching the Checked values to Transfer */
			for (var i = 0; i < temp.checkValue.length; i++)
			{
				if (temp.checkValue[i].checked == true)
				{
					table1 = table.children(i+1);
					opGrpId[i] = table1.children(0).children(0).getAttribute("value");
					opGrpName[i] = table1.children(2).children(0).getAttribute("value");
					opName[i] = table1.children(3).children(0).getAttribute("value");
					stdHrs[i] = table1.children(4).children(0).getAttribute("value");
					opnIncntv[i] = table1.children(5).children(0).getAttribute("checked");
				}
			}

			/* Manipulating the Main Table */
			for (var i = 0; i < temp.checkValue.length; i++)
			{
				if (temp.checkValue[i].checked == true)
				{
					k = obj1.children.length;
						for (var i1 = 0; i1 < count; i1++)
						{
							if ((gName[i1] == opGrpName[i]) && (oName[i1] == opName[i]))
							{
								var ans = confirm("Operation "+oName[i1]+" for OperationGroup "+gName[i1]+" is already Exist!.\n\t\t     Do u want to OverWrite!");
								if (ans == true)
								{
									obj1.children(i1).children(0).children(0).children(0).value = opGrpId[i];
									obj1.children(i1).children(0).children(2).children(0).value = opGrpName[i];
									obj1.children(i1).children(0).children(3).children(0).value = opName[i];
									obj1.children(i1).children(0).children(4).children(0).value = stdHrs[i];
									if (opnIncntv[i] == true)
									{
										obj1.children(i1).children(0).children(5).children(0).checked = true;
									}
									else
									{
										obj1.children(i1).children(0).children(5).children(0).checked = false;
									}
									t = 1;
								}
								else
					...
					if (t == 0)
					{
						for (var x = 0; x < k; x++)
						{
							if ((obj1.children(x).children(0).children(2).children(0).value == "") && (obj1.children(x).children(0).children(3).children(0).value == ""))
							{
								st[st_count] = x;
								st_count++;
							}
						}
						if ((obj1.children(st[0]).children(0).children(2).children(0).value == "") && (obj1.children(st[0]).children(0).children(3).children(0).value == ""))
						{
								obj1.children(st[0]).children(0).children(0).children(0).value = opGrpId[i];
								obj1.children(st[0]).children(0).children(2).children(0).value = opGrpName[i];
								obj1.children(st[0]).children(0).children(3).children(0).value = opName[i];
								obj1.children(st[0]).children(0).children(4).children(0).value = stdHrs[i];
								if (opnIncntv[i] == true)
								{
									obj1.children(st[0]).children(0).children(5).children(0).checked = true;
								}
								else
								{
									obj1.children(st[0]).children(0).children(5).children(0).checked = false;
								}
						}
						else
						{
							addRow(formObj);
							k = obj1.children.length;
							obj1.children(k-1).children(0).children(0).children(0).value = opGrpId[i];
							obj1.children(k-1).children(0).children(2).children(0).value = opGrpName[i];
							obj1.children(k-1).children(0).children(3).children(0).value = opName[i];
							obj1.children(k-1).children(0).children(4).children(0).value = stdHrs[i];
							if (opnIncntv[i] == true)
							{
								obj1.children(k-1).children(0).children(5).children(0).checked = true;
							}
							else
							{
								obj1.children(k-1).children(0).children(5).children(0).checked = false;
							}
						}
					}
				}
		...
		if (cnt == 0)
		{
			alert("None of the operations selected to transfer!");
			return false;
		}
	}
	
	function transOperations1(formObj)
	{
		var temp = document.forms[0];
		if (temp.checkValue != undefined)
		{
			...
			var obj1 = document.getElementById("tblIncntvYes");
			var newNode = obj1.children(0).cloneNode(true);
			var tab = obj1.children(0);
			var table = obj.children(0);
			var k = 0;
			...
			/* Manipulating the Main Table */
			//alert ("C Length: "+temp.checkValue.length);
			for (var i = 0; i < temp.checkValue.length; i++)
			{
				if (temp.checkValue[i].checked == true)
				{
					k = obj1.children.length;
						for (var i1 = 0; i1 < count; i1++)
						{
							if ((gName[i1] == opGrpName[i]) && (oName[i1] == opName[i]))
							{
								...
								{
									obj1.children(i1).children(0).children(0).children(0).value = opGrpId[i];
								//alert (obj1.children(i1).children(0).children(0).innerHTML);
									obj1.children(i1).children(0).children(2).children(0).value = opGrpName[i];
									obj1.children(i1).children(0).children(3).children(0).value = opName[i];
									t = 1;
								}
...
<body onLoad="init(); loadDefault(); existingJobs(); loadToHidden();checkRadio();setFocusPoint();hideNshow();hideOpns();">
...
<html:hidden property="custTypId"/>
	...
	<tr>
   	<td><table width="100%" cellpadding="0" cellspacing="0" id="PageTitle">
         ...
         <table>
		   <tr> 
        	...
        	<table width="100%" cellspacing="0" cellpadding="0">
          	<tr> 
            <td width="120" class="FieldTitle"><bean:message key="prodacs.job.date"/><span class="mandatory">*</span></td>
            <td width="1" class="FieldTitle">:</td>
            <td width="150" class="FieldTitle"><html:text property="date" styleClass="TextBox" size="12" readonly="true"/> 
            <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("date",JbMasterEdit.date.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
            <td width="110" class="FieldTitle"><bean:message key="prodacs.job.jobname"/><span class="mandatory">*</span></td>
            <td width="1" class="FieldTitle">:</td>
            ...
            <td class="FieldTitle"><html:text property="customerType" styleClass="TextBox" size="20" readonly="true"/></td>
            <td class="FieldTitle"><bean:message key="prodacs.customer.customername"/></td>
            <td class="FieldTitle">:</td>
            <td colspan="4" class="FieldTitle"><html:text property="customerName" styleClass="TextBox" size="40" readonly="true"/></td>
          	</tr>
          	<tr> 
		...
		<tr>
			...
			<tr id="jobOprns" style="display:none">
         	<td class="FieldTitle">
			 <%
			 ...
			 %>
               ...
               <tr> 
	                 ...
	                 <input type="checkbox" name="CheckAll" value="checkbox" onClick="chkAll(document.forms[0]);"></td>
               ...
               <tr> 
                  ...
                  <td width="107" class="TableItems"><input readonly name="serial" class="TextBoxFull" size="9" maxlength="10"></td>
                  <td width="222" class="TableItems"><input readonly name="opnGrpName" class="TextBoxFull" size="10" maxlength="50"></td>
                  <td width="172" class="TableItems"><html:text property="jobOperationName" styleClass="thinTBox" onfocus="opnName(this);"/></td>
			  ...
			  <tr>
					...
					<td colspan="2"><input type="text" readonly name="totStdHrs" size="10" class="TextBox" onkeypress="return isNumber(this);" onkeyup="return isNumber(this);"/></td>
			  </tr>
			  </table>
              <table width="100%" cellpadding="0" cellspacing="0" id="BtnBg">
                <tr>
                  <td><html:button property="saveOperations" styleClass="Button" onclick="submitItem();">Modify</html:button></td>
                </tr>
              </table>
              </fieldset>
			 <%
			 ...
			 %>
               ...
               <tr> 
                  ...
                  <td width="172" class="TableItems"><html:text property="jobOperationName" styleClass="thinTBox" onfocus="opnNameIncntnvYes(this);"/></td>
               </tr>
              </table>
              <table width="100%" cellpadding="0" cellspacing="0" id="BtnBg">
                <tr>
                  <td><html:button property="saveOperations" styleClass="Button" onclick="submitItemIncntvYes();">Modify</html:button></td>
                </tr>
              </table>
              </fieldset>
			<%
       	...
       	<tr>
			...
			<br>
					...
					<table width = "100%" cellspacing = "0" cellpadding="0" id = "BtnBg">
						<tr> 
						  <td><html:button property="transfer" styleClass="Button" value = "Transfer" onclick="transOperations(document.forms[0]);"/></td>
						</tr>
					</table>
				</table>
				<%
