...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.employee.EmployeeTypeListForm" />
...
<% 
	if (BuildConfig.DMODE)
		System.out.println("Employee Type List");
	...
	if(sortFieldName.equals("EMP_TYP_NAME"))
	{
		columnNumber=1;
	}
	else if(sortFieldName.equals("EMP_TYP_DT"))
	{
		columnNumber=2;
	}
	else if(sortFieldName.equals("EMP_TYP_OT"))
	{
		columnNumber=3;
	}
	else if(sortFieldName.equals("EMP_TYP_INCENTIVE"))
	{
		columnNumber=4;
	}

	
	if(frm.getSortOrder().equalsIgnoreCase("desending"))
	{
		order = true;
	}
	else
...
/* 	Creating the Home and Remote Objects 	*/
		...
		{  
		  ...
		  if (frm.getViewValidEntries().equalsIgnoreCase("yes"))
		  {
		  		...
		  		temp.setFieldName("EMP_TYP_ISVALID");
		  ...
		  }else if (frm.getViewValidEntries().equalsIgnoreCase("no"))
		  {
			...
			if (!frm.getSearchText().equals(""))
			{
	   		...
	   		tempFilter.setFieldName("EMP_TYP_NAME");
			   tempFilter.setFieldValue(frm.getSearchText());
			   String specialFunction="";
			   if(frm.getEmployeeType().equals("0"))
			   {
				   specialFunction = "Starts With";
			   }
			   else if(frm.getEmployeeType().equals("1"))
			   {
				   specialFunction = "Ends With";
			   }
			   else if(frm.getEmployeeType().equals("2"))
			   {
				   specialFunction = "Exactly";
			   }
			   else if(frm.getEmployeeType().equals("3"))
			   {
	   ...
	   int pagecount = Integer.parseInt(frm.getPage());
		...
		if (BuildConfig.DMODE)
 		...
 		HashMap empTypeDet = empObj.getAllEmployeeTypeDetails(objFilter, frm.getSortField(),order,((pageCount-1)*maxItems)+1,maxItems);
		
		Vector empTypeDetails = (Vector)empTypeDet.get("EmployeeTypeDetails");
		if (BuildConfig.DMODE)
			System.out.println("Emp Details Obj: "+empTypeDet);
		pageContext.setAttribute("empTypeDetails",empTypeDetails);
	  	totalCount = Integer.parseInt((empTypeDet.get("TotalRecordCount")).toString());
	  	if (BuildConfig.DMODE)
...
<script language="Javascript" type="text/Javascript">
	function viewEmployeeType(val)
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmEmpTypeEdit.do?formAction=view&id='+val;
	...
	function editItem()
	{
		if(modifyRow('empTypeList', document.forms[0]))
		{
		   var val = getCheckedValueDetail('empTypeList', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmEmpTypeEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	function makeInvalidItem()
	{
		if (validRow('empTypeList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmEmpTypeEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('empTypeList', document.forms[0]);
			var subQuery = '';
	...
	function makeValid()
	{
		var queryString = '<bean:message key="context"/>/frmEmpTypeEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('empTypeList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	function reLoadData()
	{
		...
		{
			<%
				String srchEmpTypName = request.getParameter("searchText");
				session.setAttribute("searchText",srchEmpTypName);
				String srchEmpType = request.getParameter("employeeType");
				session.setAttribute("employeeType",srchEmpType);
		...
		{
			if ('<%= request.getParameter("empTypName") %>' != 'null')
				temp.searchText.value = '<%= request.getParameter("empTypName") %>';
			else
				temp.searchText.value = "";

			if ('<%= request.getParameter("empType") %>' != 'null')
				temp.employeeType.value = '<%= request.getParameter("empType") %>';
			else
...
<body onLoad = "loadDefault(); sortByTable('empTypeList',<%=columnNumber%>,'<%=order%>'); reLoadData();">
<html:form action="frmEmpTypeList">
...
<table width="100%" height="100%" cellpadding="10" cellspacing="0">
  <tr>
    <td valign="top"><table width="100%" cellpadding="0" cellspacing="0" id="PageTitle">
        <tr>
            <td><bean:message key="prodacs.employee.employeetypelist.header"/></td>
        </tr>
      </table>
        <br>
        <table width="100" cellspacing="0" cellpadding="0" align="right">
 		...
 		<menuconfig:userRights url="#" id="#" onMouseOver="window.status='Modify EmployeeType Info'; return true"  onMouseOut="window.status=''; return true" resourceId="1003" text="[ Modify ]" classId="TopLnk" onClick="javaScript:editItem();"/>
 		<menuconfig:userRights url="#" id="mkInValid" onMouseOver="window.status='Make EmployeeType Info InValid'; return true"  onMouseOut="window.status=''; return true" resourceId="1003" text="[ MakeInValid ]" classId="TopLnk" onClick="javaScript:makeInvalidItem();"/>
        ...
        <table width="100%" cellspacing="0" cellpadding="0" id="Filter">
          <tr> 
            <td><bean:message key="prodacs.employee.employeetype"/>
        ...
        <br>
        <table width="100%" cellpadding="0" cellspacing="0" id="empTypeList">
          <tr> 
            <td width="25" class="SortHeader"><input name="CheckAll" type="checkbox" id="CheckAll" value="checkbox" onClick="checkAll(document.EmployeeTypeList)"></td>
            <td width="250" class="SortHeader" onClick="sortTables(document.forms[0],this,'EMP_TYP_NAME')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.employee.employeetype"/></td>
            <td width="150" class="SortHeader" onClick="sortTables(document.forms[0],this,'EMP_TYP_DT')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.payroll.duty"/></td>
            <td width="150" class="SortHeader" onClick="sortTables(document.forms[0],this,'EMP_TYP_OT')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.payroll.overtime"/></td>
            <td class="SortHeader" onClick="sortTables(document.forms[0],this,'EMP_TYP_INCENTIVE')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.job.incentive"/></td>
          </tr>
          <logic:iterate id="bt1" name="empTypeDetails" indexId="count">
          <tr>
      <%
	      if(count.intValue()%2 == 0) 
   	   {
     	%>
				<logic:equal name="bt1" property="emp_Typ_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="emp_Typ_IsValid" value="1"><td  class="TableItems"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="emp_Typ_Id"/>' onClick="isSimilar(this)"></td>
            <td width="250" class="TableItems"><a href="#" onMouseOver='window.status="View Employee Type List Info";return true;'onMouseOut="window.status='';return true" id='<bean:write name="bt1" property="emp_Typ_Id"/>' onclick="viewEmployeeType(this.id);"><bean:write name="bt1" property="emp_Typ_Name"/>&nbsp;</a></td>
            <td width="150" class="TableItems" align="center">
				<logic:equal name="bt1" property="emp_Typ_Dt" value="false"><img src='<bean:message key="context"/>/images/inactive.gif'></logic:equal>
				<logic:equal name="bt1" property="emp_Typ_Dt" value="true"><img src='<bean:message key="context"/>/images/active.gif'></logic:equal>&nbsp;
			</td>
            <td width="150" class="TableItems" align="center">
				<logic:equal name="bt1" property="emp_Typ_Ot" value="false"><img src='<bean:message key="context"/>/images/inactive.gif'></logic:equal>
				<logic:equal name="bt1" property="emp_Typ_Ot" value="true"><img src='<bean:message key="context"/>/images/active.gif'></logic:equal>&nbsp;
			</td>
            <td class="TableItems" align="center">
				<logic:equal name="bt1" property="emp_Typ_Incentive" value="false"><img src='<bean:message key="context"/>/images/inactive.gif'></logic:equal>
				<logic:equal name="bt1" property="emp_Typ_Incentive" value="true"><img src='<bean:message key="context"/>/images/active.gif'></logic:equal>&nbsp;
			</td>
		<%
			} else
			{
		%>            
				<logic:equal name="bt1" property="emp_Typ_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="emp_Typ_IsValid" value="1"><td  class="TableItems2"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="emp_Typ_Id"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems2"><a href="#" onMouseOver="window.status='View Employee Type List Info';return true" onMouseOut="window.status='';return true" id='<bean:write name="bt1" property="emp_Typ_Id"/>' onclick="viewEmployeeType(this.id);"><bean:write name="bt1" property="emp_Typ_Name"/>&nbsp;</a></td>
            <td width="150" class="TableItems2" align="center">
				<logic:equal name="bt1" property="emp_Typ_Dt" value="false"><img src='<bean:message key="context"/>/images/inactive.gif'></logic:equal>
				<logic:equal name="bt1" property="emp_Typ_Dt" value="true"><img src='<bean:message key="context"/>/images/active.gif'></logic:equal>&nbsp;
			</td>
            <td width="150" class="TableItems2" align="center">
