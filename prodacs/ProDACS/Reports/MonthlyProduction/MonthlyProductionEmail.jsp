...
<%@ page import="com.savantit.prodacs.businessimplementation.reports.MonthlyPercentageDetails"%>
...
<useradmin:userrights resource="32"/>
<%
	if (BuildConfig.DMODE)
		System.out.println("Monthly Production Starts...");
	EJBLocator obj = new EJBLocator(); // Create an Object for the EJBLocator
	MonthlyPercentageDetails[] objMonthlyPercentageDetails;
	...
	if ((month != "") && (year != ""))
	{
		try
		{
			...
			objMonthlyPercentageDetails = monthlyObj.fetchMonthlyPercentageOfMachine(machineCode,Integer.parseInt(month), Integer.parseInt(year));
			
			dataSource = new JRBeanArrayDataSource(objMonthlyPercentageDetails);
			...
			mailAttachmentName = "Monthly Production of Machine "+machineCode+" for "+strMonth+", "+year+".pdf";
			parameters.put("Title", "Monthly Production of Machine "+machineCode+" for "+strMonth+", "+year);
			parameters.put("month",month);
			parameters.put("year",year);
			parameters.put("machineCode",machineCode);
			
			is = this.getClass().getClassLoader().getResourceAsStream("/reports/MonthlyProduction.jasper");
	...
	<script>
	  ...
	  function sendMailTo()
	  {
	  	if(isValidate())
		{
			...
			document.forms[0].action='<bean:message key="context"/>/Reports/MonthlyProduction/MonthlyProductionEmail.jsp';
	  ...
	  {
		document.forms[0].action='<bean:message key="context"/>/Reports/MonthlyProduction/MonthlyProduction.jsp';
		document.forms[0].submit();
	  }
	   
	   
	  </script>
	<form name="frmMonthlyProductionEmail">
<%
	if (BuildConfig.DMODE)
...
<%
	...
	{
%>
	<jsp:forward page='MonthlyProduction.jsp'>
