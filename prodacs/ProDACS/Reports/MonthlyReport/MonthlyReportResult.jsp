...
<%@ page import="com.savantit.prodacs.businessimplementation.reports.MonthlyReportDetails"%>
<%@ page import="com.savantit.prodacs.util.EJBLocator"%>
<%@ page import="javax.rmi.PortableRemoteObject" %>
<useradmin:userrights resource="31"/>
<%
	if (BuildConfig.DMODE)
		System.out.println("Monthly Report Result Starts...");
	EJBLocator obj = new EJBLocator(); // Create an Object for the EJBLocator
	MonthlyReportDetails[] objMonthlyReportDetails;
	...
	if (BuildConfig.DMODE)
		System.out.println("Month,Year&Validator : "+ month+","+year+"&"+validator);
	...
	try
	{
		if ((month != "") && (year != "") && (validator==null))
		{
			objMonthlyReportDetails = monthlyObj.fetchMonthlyReportsDetails(Integer.parseInt(month), Integer.parseInt(year));
			dataSource = new JRBeanArrayDataSource(objMonthlyReportDetails);
			...
			parameters.put("Title", "Monthly Report for "+strMonth+" ' "+year);
			parameters.put("month",month);
			parameters.put("year",year);

			is = this.getClass().getClassLoader().getResourceAsStream("/reports/MonthlyReport.jasper");
			jr = (JasperReport)JRLoader.loadObject(is);
		}
		else
		{
			objMonthlyReportDetails = monthlyObj.fetchMonthlyReportsDetailsValidationReport(Integer.parseInt(month), Integer.parseInt(year));
			dataSource = new JRBeanArrayDataSource(objMonthlyReportDetails);
			...
			parameters.put("Title", "Monthly Validation Report for "+strMonth+" ' "+year);
			parameters.put("month",month);
			parameters.put("year",year);
	...
	<script>
		...
		function pdfReport()
		{
			...
			document.forms[0].action='<bean:message key="context"/>/Reports/MonthlyReport/MonthlyReportResult.jsp';
			document.forms[0].submit();
		}
		
		function closeReport()
		{
			document.forms[0].action='<bean:message key="context"/>/Reports/MonthlyReport/MonthlyReport.jsp';
			document.forms[0].submit();
		}
		
		function emailReport()
		{
			document.forms[0].action='<bean:message key="context"/>/Reports/MonthlyReport/MonthlyReportEmail.jsp';
			document.forms[0].submit();		  
		}
		
	</script>
	<body>
	<form name="frmMonthlyReportResult">
