...
<%@ page import="com.savantit.prodacs.businessimplementation.reports.MachineUtilizationReportDetails"%>
...
<%
	if (BuildConfig.DMODE)
		System.out.println("Machine Utilization Email Starts...");
	EJBLocator obj = new EJBLocator(); // Create an Object for the EJBLocator
	MachineUtilizationReportDetails[] objMachineUtilizationReportDetails;
	...
	if ((startDate != "") && (endDate != ""))
	{
		...
		mailAttachmentName = "Machine_Utilization_Report_From_"+startDate+"_To_"+endDate+".pdf";
		try
		{
			...
			SessionReportsDetailsManagerHome machineHomeObj = (SessionReportsDetailsManagerHome)PortableRemoteObject.narrow(obj.getHome(),SessionReportsDetailsManagerHome.class);
			SessionReportsDetailsManager machineObj = (SessionReportsDetailsManager)PortableRemoteObject.narrow(machineHomeObj.create(),SessionReportsDetailsManager.class);
			
			objMachineUtilizationReportDetails = machineObj.fetchMachineUtilization(ge.getTime(), ge1.getTime());
			
			dataSource = new JRBeanArrayDataSource(objMachineUtilizationReportDetails);
			parameters.put("Title", "Machine Utilization Report From "+startDate+" To "+endDate);
	...
	<script>
	  ...
	  function sendMailTo()
	  {
	  	if(isValidate())
		{
			...
			document.forms[0].action='<bean:message key="context"/>/Reports/MachineUtilization/MachineUtilizationEmail.jsp';
	  ...
	  {
		document.forms[0].action='<bean:message key="context"/>/Reports/MachineUtilization/MachineUtilization.jsp';
		document.forms[0].submit();
	  }
	   
	   
	  </script>
	<form name="frmMachineUtilizationEmail">
<%
	if (BuildConfig.DMODE)
...
<%
	...
	{
%>
	<jsp:forward page='MachineUtilization.jsp'>
