...
<%
	if (BuildConfig.DMODE)
		System.out.println("Production of Machine Result Starts.");
	...
	if ((startDate != "") && (endDate != ""))
	{
		...
		try
		{
			...
			SessionReportsDetailsManagerHome proMacHomeObj = (SessionReportsDetailsManagerHome)PortableRemoteObject.narrow(obj.getHome(),SessionReportsDetailsManagerHome.class);
			SessionReportsDetailsManager proMacObj = (SessionReportsDetailsManager)PortableRemoteObject.narrow(proMacHomeObj.create(),SessionReportsDetailsManager.class);
			
			objProductionOfMachineDetails = proMacObj.fetchProductionOfMachineDetails(ge.getTime(),ge1.getTime(),machineCode);
			if (BuildConfig.DMODE)
				System.out.println("objProductionOfMachineDetails: "+ objProductionOfMachineDetails.length);
			dataSource = new JRBeanArrayDataSource(objProductionOfMachineDetails);		
			
			//File is = new File(application.getRealPath("/reports/QuantityProduced.jasper"));
...
%>
		...
		<script>
			...
			function pdfReport()
			{
				...
				document.forms[0].action='<bean:message key="context"/>/Reports/ProductionOfMachine/ProductionOfMachineResult.jsp';
