...
<%@ page import="com.savantit.prodacs.businessimplementation.reports.TopSheetDetails"%>
...
<useradmin:userrights resource="38"/>
<%
	if (BuildConfig.DMODE)
		System.out.println("TopSheet Email Starts...");
	EJBLocator obj = new EJBLocator(); // Create an Object for the EJBLocator
	TopSheetDetails[] objTopSheetDetails;
	...
	try
	{
		...
		SessionReportsDetailsManagerHome topSheetHomeObj = (SessionReportsDetailsManagerHome)PortableRemoteObject.narrow(obj.getHome(),SessionReportsDetailsManagerHome.class);
		SessionReportsDetailsManager topSheetObj = (SessionReportsDetailsManager)PortableRemoteObject.narrow(topSheetHomeObj.create(),SessionReportsDetailsManager.class);
		
		objTopSheetDetails = topSheetObj.fetchTopSheetDetails();
		
		dataSource = new JRBeanArrayDataSource(objTopSheetDetails);

		mailAttachmentName = "TopSheet.pdf";

		is = this.getClass().getClassLoader().getResourceAsStream("/reports/TopSheet.jasper");
	...
	<script>
	  ...
	  function sendMailTo()
	  {
	  	if(isValidate())
		{
			...
			document.forms[0].action='<bean:message key="context"/>/Reports/TopSheet/TopSheetEmail.jsp';
	  ...
	  {
		document.forms[0].action='<bean:message key="context"/>/Reports/TopSheet/TopSheet.jsp';
		document.forms[0].submit();
	  }
	   
	   
	  </script>
	<form name="frmTopSheetEmail">
<%
	if (BuildConfig.DMODE)
