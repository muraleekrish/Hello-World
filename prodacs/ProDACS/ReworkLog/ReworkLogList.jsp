...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.reworklog.ReworkLogListForm" />
...
<% 
	if (BuildConfig.DMODE)
		System.out.println("Rework Log List");
	...
	if(sortFieldName.equals("RWK_CATEGORY"))
	{
		columnNumber=1;
	}
	else if(sortFieldName.equals("RWK_RSN"))
	{
		columnNumber=2;
	}
	...
	else if(sortFieldName.equals("RWK_DATE"))
	{
		columnNumber=4;
	}
	...
	try
	  {
		...
		if(frm.getFormAction().equalsIgnoreCase("add") || frm.getFormAction().equalsIgnoreCase(""))
		{  
  		...
  		else if(frm.getFormAction().equalsIgnoreCase("search"))
  		{
			...
			if (frm.getListValidEntries().equalsIgnoreCase("yes"))
			{
				...
				temp.setFieldName("RWKLOG_ISVALID");
			...
			if (!frm.getListJobName().equals(""))
			{
				Filter tempFilter = new Filter();
				tempFilter.setFieldName("JB_NAME");
			   	tempFilter.setFieldValue(frm.getListJobName());
			   	String specialFunction="";
			   	if(frm.getJobSoryBy().equals("0"))
			   	{
					specialFunction = "Starts With";
			   	}
			   	else if(frm.getJobSoryBy().equals("1"))
			   	{
					specialFunction = "Ends With";
			   	}
			   	else if(frm.getJobSoryBy().equals("2"))
			   	{
					specialFunction = "Exactly";
			   	}
			   	else if(frm.getJobSoryBy().equals("3"))
			   	{
			...
			if ((!frm.getRwkLogFromDate().equalsIgnoreCase("")) && (!frm.getRwkLogToDate().equalsIgnoreCase("")))
			{
				String[] month = {"JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"};
				StringTokenizer frmDate = new StringTokenizer((frm.getRwkLogFromDate()).trim(),"-");
				...
				StringTokenizer toDate = new StringTokenizer((frm.getRwkLogToDate()).trim(),"-");
				...
				String rwkDate = proFDate  + "$" + proTDate;
				if (BuildConfig.DMODE)
					System.out.println("Rework Date: "+ rwkDate);
				Filter temp = new Filter();
				temp.setFieldName("RWK_DATE");
				temp.setFieldValue(rwkDate);
		...
		int maxItems = Integer.parseInt(frm.getMaxItems());
 		...
 		HashMap rwkDetails = rwkLogObj.getAllReworkLogDetails(objFilter, frm.getSortField(), order, ((pageCount-1)*maxItems)+1, maxItems);
		if (BuildConfig.DMODE)
			System.out.println("Rwk Details (HM): "+rwkDetails);
			
		/* Hashmap for loading JobNames 
		HashMap jobNames = rwkLogObj.;
		System.out.println("JobNames (HM): "+jobNames); */
			
	 	Vector rwkDet = (Vector)rwkDetails.get("ReworkLogDetails");

	 	pageContext.setAttribute("rwkDet",rwkDet); 
	 	/* pageContext.setAttribute("customerTypes",customerTypes); */
	 	
	  	int totalCount = Integer.parseInt((rwkDetails.get("TotalRecordCount")).toString()); 
...
<script language = "Javascript" language="text/Javascript">
	...
	{
		document.forms[0].action = '<bean:message key="context"/>/frmRewrkLogEdit.do?formAction=view&id='+val;
		document.forms[0].submit();
	}
	
	function makeInvalid()
	{
		if (validRow('reWrkLogList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmRewrkLogEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('reWrkLogList', document.forms[0]);
			var subQuery = '';
	...
	function makeValid()
	{
		var queryString = '<bean:message key="context"/>/frmRewrkLogEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('reWrkLogList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	{
		temp = document.forms[0];
		temp.action = '<bean:message key="context"/>/ReworkLog/ReworkLogAdd.jsp';
		temp.submit();
	}
	...
	function reLoadData()
	{
		...
		{
			<%
				String srchFrmDate = request.getParameter("rwkLogFromDate");
				session.setAttribute("rwkLogFromDate",srchFrmDate);
				String srchToDate = request.getParameter("rwkLogToDate");
				session.setAttribute("rwkLogToDate",srchToDate);
				String srchJob = request.getParameter("listJobName");
				session.setAttribute("listJobName",srchJob);
				String srchJobCombo = request.getParameter("jobSoryBy");
				session.setAttribute("jobSoryBy",srchJobCombo);
		...
		else if (temp.formAction.value == "afterView") 
		{
			if ('<%= request.getParameter("fromDate") %>' != 'null')
				temp.rwkLogFromDate.value = '<%= request.getParameter("fromDate") %>';
			else
				temp.rwkLogFromDate.value = "";

			if ('<%= request.getParameter("toDate") %>' != 'null')
				temp.rwkLogToDate.value = '<%= request.getParameter("toDate") %>';
			else
				temp.rwkLogToDate.value = "";

			if ('<%= request.getParameter("jbName") %>' != 'null')
				temp.listJobName.value = '<%= request.getParameter("jbName") %>';
			else
				temp.listJobName.value = "0";

			if ('<%= request.getParameter("srchJbName") %>' != 'null')
				temp.jobSoryBy.value = '<%= request.getParameter("srchJbName") %>';
			else
				temp.jobSoryBy.value = "0";
...
<body onLoad="init(); loadDefault();sortByTable('rewrkLogList',<%= columnNumber%>,'<%= order%>'); reLoadData();">
<html:form action="frmRewrkLogList">
...
<html:hidden property="sortOrder" value="<%= frm.getSortOrder()%>"/>
	...
	<td>
		...
		<tr>
			<td><bean:message key="prodacs.reworklog.reworklog"/></td>
		</tr>
		</table>
	<br>
		...
		<tr> 
	 		...
	 		<menuconfig:userRights url="#" id="mkInValid" onMouseOver="window.status='Make ReworkLog Info InValid'; return true"  onMouseOut="window.status=''; return true" resourceId="1027" text="[ MakeInValid ]" classId="TopLnk" onClick="javaScript:makeInvalid();"/>
	 		<menuconfig:userRights url="#" id="mkValid" onMouseOver="window.status='Make ReworkLog Info Valid'; return true"  onMouseOut="window.status=''; return true" resourceId="1027" text="[ MakeValid ]" classId="TopLnk" style="display:none" onClick="javaScript:makeValid();"/>
		</tr>
		</table>
	<br>
		...
		<table width="100%" cellspacing="0" cellpadding="0" id="Filter">
          	<tr> 
	            ...
	            <td width="1">:</td>
     			<td width="200"><html:text property="rwkLogFromDate" styleClass="TextBox" size="12" readonly="true"/>
            	  <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("rwkLogFromDate",ReworkLogList.rwkLogFromDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
			<td width="100"></td>
	            <td width="80"><bean:message key="prodacs.workorder.todate"/></td>
     			<td width="1">:</td>
            	<td width="250"><html:text property="rwkLogToDate" styleClass="TextBox" size="12" readonly="true"/>
	              <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("rwkLogToDate",ReworkLogList.rwkLogToDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
			</tr>
			<tr>
		      <td><bean:message key="prodacs.job.jobname"/> </td>
			  <td>:</td>
			  <td>	<html:text property="listJobName" styleClass="TextBox" value="" size="15"/>
            		<html:select property="jobSoryBy" styleClass="Combo">
	...
	<br>
		<table width="100%" cellpadding="0" cellspacing="0" id="rewrkLogList">
		<tr> 
			<td width="25" class="SortHeader"><input name="CheckAll" type="checkbox" id="CheckAll" value="checkbox" onClick="checkAll(document.ReworkLogList)"></td>
			<td width="150" class="SortHeader" onClick="sortTables(document.forms[0],this,'RWK_CATEGORY')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"><bean:message key="prodacs.rework.reworkcategory"/></td>
			<td onClick="sortTables(document.forms[0],this,'RWK_RSN')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"><bean:message key="prodacs.rework.reworkreason"/></td>
			<td width="130" onClick="sortTables(document.forms[0],this,'WO_NO')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"><bean:message key="prodacs.workorder.workorder"/></td>
			<td width="130" onClick="sortTables(document.forms[0],this,'JB_NAME')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"><bean:message key="prodacs.job.jobname"/></td>
			<td width="130" onClick="sortTables(document.forms[0],this,'RWK_DATE')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"><bean:message key="prodacs.rework.reworkdate"/></td>
		</tr>
		<logic:iterate id="bt1" name="rwkDet" indexId="count">
       	<tr>
<%
	if(count.intValue()%2 == 0)
	{
%>
			<logic:equal name="bt1" property="rwkIsvalid" value="false"><td class="InValid"></logic:equal>
			<logic:equal name="bt1" property="rwkIsvalid" value="true"><td class="TableItems"></logic:equal>	
	            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="rwkLogId"/>' onClick="isSimilar(this)"></td>
	            <td class="TableItems"><a href="#" onMouseOver="window.status='View ReworkLog Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="rwkLogId"/>' onClick="viewItem(this.id);"><bean:write name="bt1" property="rwkCategory"/>&nbsp;</a></td>
	            <td class="TableItems"><bean:write name="bt1" property="rwkReason"/>&nbsp;</td>
	            <td class="TableItems"><bean:write name="bt1" property="woNo"/>&nbsp;</td>
	            <td class="TableItems"><bean:write name="bt1" property="jbName"/>&nbsp;</td>
	            <td class="TableItems"><bean:define id="rwkDate" name="bt1" property="rwkDate"/><%= rwkDate.toString().substring(0,10) %>&nbsp;</td>
<%
	} else
	{
%>
			<logic:equal name="bt1" property="rwkIsvalid" value="false"><td class="InValid"></logic:equal>
			<logic:equal name="bt1" property="rwkIsvalid" value="true"><td class="TableItems2"></logic:equal>	
			<input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="rwkLogId"/>' onClick="isSimilar(this)"></td>
			<td class="TableItems2"><a href="#" onMouseOver="window.status='View ReworkLog Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="rwkLogId"/>' onClick="viewItem(this.id);"><bean:write name="bt1" property="rwkCategory"/>&nbsp;</a></td>
			<td class="TableItems2"><bean:write name="bt1" property="rwkReason"/>&nbsp;</td>
			<td class="TableItems2"><bean:write name="bt1" property="woNo"/>&nbsp;</td>
			<td class="TableItems2"><bean:write name="bt1" property="jbName"/>&nbsp;</td>
			<td class="TableItems2"><bean:define id="rwkDate1" name="bt1" property="rwkDate"/><%= rwkDate1.toString().substring(0,10) %>&nbsp;</td>
<%
	}
%>
		</tr>
