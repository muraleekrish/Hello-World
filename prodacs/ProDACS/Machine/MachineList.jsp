...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.machine.MachineListForm" />
...
<% 
	...
	else if(sortFieldName.equals("MC_NAME"))
	{
		columnNumber=2;
	}
	else if(sortFieldName.equals("MC_TYP_NAME"))
	{
		columnNumber=3;
	}
	else if(sortFieldName.equals("MC_INSTALLDATE"))
	{
		columnNumber=4;
	}
...
/* 	Creating the Home and Remote Objects 	*/
		...
		{  
		  ...
		  if (frm.getMachineValidEntries().equalsIgnoreCase("yes"))
		  {
		  		Filter temp = new Filter();
		  		temp.setFieldName("MC_ISVALID");
		  		temp.setFieldValue("0");
		  		filter.add(temp);
		  }else if (frm.getMachineValidEntries().equalsIgnoreCase("no"))
		  {
		  		Filter temp = new Filter();
		  		temp.setFieldName("MC_ISVALID");
			...
			if (!frm.getMachineType().equalsIgnoreCase("0"))
			{
				Filter temp = new Filter();
		  		temp.setFieldName("MC_TYP_ID");
		  		temp.setFieldValue(frm.getMachineType());
		  		filter.add(temp);
			}
			
			if (!frm.getMachineName().equals(""))
			{
   			Filter tempFilter = new Filter();
	   		tempFilter.setFieldName("MC_CDE");
			   tempFilter.setFieldValue(frm.getMachineName());
			   String specialFunction="";
			   if(frm.getMachineSearchTab().equals("0"))
			   {
				   specialFunction = "Starts With";
			   }
			   else if(frm.getMachineSearchTab().equals("1"))
			   {
				   specialFunction = "Ends With";
			   }
			   else if(frm.getMachineSearchTab().equals("2"))
			   {
				   specialFunction = "Exactly";
			   }
			   else if(frm.getMachineSearchTab().equals("3"))
			   {
	   ...
	   int pagecount = Integer.parseInt(frm.getPage());
 		...
 		HashMap macDet = machineObj.getAllMachineDetails(objFilter, frm.getSortField(),order,((pageCount-1)*maxItems)+1,maxItems);
		if (BuildConfig.DMODE)
			System.out.println("Machine Details(HM): "+ macDet);

		Vector machineDetails = (Vector)macDet.get("MachineDetails");
		pageContext.setAttribute("machineDetails",machineDetails);

	  	totalCount = Integer.parseInt((macDet.get("TotalRecordCount")).toString());
	  	if (BuildConfig.DMODE)
	  		System.out.println("Total Record Count: "+ totalCount);

	 	HashMap machineType = machineObj.getAllMachineTypes();
	  	pageContext.setAttribute("machineType",machineType);
...
<html:html>
<head>
<title><bean:message key="prodacs.machine.machinelist.header"/></title>
...
<script language="Javascript" type="text/Javascript">
	...
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmMachEdit.do?formAction=view&id='+val;
	   document.forms[0].submit();
	}
	
	function editItem()
	{
		if(modifyRow('machInfoList', document.forms[0]))
		{
		   var val = getCheckedValueDetail('machInfoList', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmMachEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	...
	{
		if (validRow('machInfoList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmMachEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('machInfoList', document.forms[0]);
			var subQuery = '';
	...
	{
		var queryString = '<bean:message key="context"/>/frmMachEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('machInfoList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	function reLoadData()
	{
		...
		{
			<%
				String srchMachName = request.getParameter("machineName");
				session.setAttribute("machineName",srchMachName);
				String srchMachCombo = request.getParameter("machineSearchTab");
				session.setAttribute("machineSearchTab",srchMachCombo);
				String srchMachType = request.getParameter("machineType");
				session.setAttribute("machineType",srchMachType);
				String viewAll = request.getParameter("machineValidEntries");
				session.setAttribute("machineValidEntries",viewAll);
			%>
		}
		else if (temp.formAction.value == "afterView") 
		{
			if ('<%= request.getParameter("machName") %>' != 'null')
				temp.machineName.value = '<%= request.getParameter("machName") %>';
			else
				temp.machineName.value = "";

			if ('<%= request.getParameter("machSrchTab") %>' != 'null')
				temp.machineSearchTab.value = '<%= request.getParameter("machSrchTab") %>';
			else
				temp.machineSearchTab.value = "0";

			if ('<%= request.getParameter("machineType") %>' != 'null')
				temp.machineType.value = '<%= request.getParameter("machineType") %>';
			else
				temp.machineType.value = "0";

			if ('<%= request.getParameter("viewAll") %>' != 'null')
				temp.machineValidEntries.value = '<%= request.getParameter("viewAll") %>';
			else
				temp.machineValidEntries.value = "0";
		}
	}
...
<body onLoad="loadDefault(); sortByTable('machInfoList',<%=columnNumber%>,'<%=order%>'); reLoadData();">
<html:form action="frmMachInfoList">
...
<table width="100%" height="100%" cellpadding="10" cellspacing="0">
  <tr>
    <td valign="top"><table width="100%" cellpadding="0" cellspacing="0" id="PageTitle">
        <tr>
            <td><bean:message key="prodacs.machine.machinelist.header"/></td>
        </tr>
      </table>
        <br>
        <table width="100" cellspacing="0" cellpadding="0" align="right">
 		...
 		<menuconfig:userRights url="#" id="mkValid" onMouseOver="window.status='Make Machine Info Valid'; return true"  onMouseOut="window.status=''; return true" resourceId="1006" text="[ MakeValid ]" classId="TopLnk" style="display:none" onClick="javaScript:makeValidItem();"/>
          </tr>
        </table>
        <br>
        ...
        <table width="100%" cellspacing="0" cellpadding="0" id="Filter">
          <tr> 
            <td><bean:message key="prodacs.machine.machinecode"/>
              <html:text property="machineName" styleClass="TextBox"/>
            <html:select property="machineSearchTab" styleClass="Combo">
              ...
              <bean:message key="prodacs.machine.machinetype"/>
              <html:select property="machineType" styleClass="Combo">
                <html:option value="0">-- Machine Type --</html:option>
  					 <html:options collection="machineType" property="key" labelProperty="value"/>
              </html:select>&nbsp;&nbsp;&nbsp;
              <bean:message key="prodacs.common.view"/>
              <html:select property="machineValidEntries" styleClass="Combo">
        ...
        <br>
        <table width="100%" cellpadding="0" cellspacing="0" id="machInfoList">
          <tr> 
            <td width="25" class="SortHeader"><input name="CheckAll" type="checkbox" id="CheckAll" value="checkbox" onClick="checkAll(document.MachineList)"></td>
            <td width="130" onClick="sortTables(document.forms[0],this,'MC_CDE')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.machine.machinecode"/></td>
            <td class="SortHeader" onClick="sortTables(document.forms[0],this,'MC_NAME')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> <bean:message key="prodacs.machine.machinename"/></td>
            <td width="120" onClick="sortTables(document.forms[0],this,'MC_TYP_NAME')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> <bean:message key="prodacs.machine.machinetype"/></td>
            <td width="200" onClick="sortTables(document.forms[0],this,'MC_INSTALLDATE')" class="SortHeader"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> <bean:message key="prodacs.machine.installationdate"/></td>
          </tr>
          <logic:iterate id="bt1" name="machineDetails" indexId="count">
          <tr>
      <%
	      if(count.intValue()%2 == 0) 
   	   {
     	%>
				<logic:equal name="bt1" property="mach_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="mach_IsValid" value="1"><td  class="TableItems"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="mach_Cde"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems"><a href="#" onMouseOver='window.status="View Machine Info";return true;' onMouseOut="window.status='';return true;" id='<bean:write name="bt1" property="mach_Cde"/>' onClick="viewItem(this.id)"><bean:write name="bt1" property="mach_Cde"/>&nbsp;</a></td>
            <td class="TableItems"><bean:write name="bt1" property="mach_Name"/>&nbsp;</td>
            <td class="TableItems"><bean:write name="bt1" property="mach_Typ_Name"/>&nbsp;</td>
            <td class="TableItems">
            <logic:notEmpty name="bt1" property="mach_Install_Date">
		<bean:define id="macInsDate" name="bt1" property="mach_Install_Date"/><%= macInsDate.toString().substring(0,10) %>
	   </logic:notEmpty>&nbsp;</td>
		<%
			} else
			{
		%>            
				<logic:equal name="bt1" property="mach_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="mach_IsValid" value="1"><td  class="TableItems2"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="mach_Cde"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems2"><a href="#" onMouseOver='window.status="View Machine Info";return true;' onMouseOut="window.status='';return true;" id='<bean:write name="bt1" property="mach_Cde"/>' onClick="viewItem(this.id)"><bean:write name="bt1" property="mach_Cde"/>&nbsp;</a></td>
            <td class="TableItems2"><bean:write name="bt1" property="mach_Name"/>&nbsp;</td>
            <td class="TableItems2"><bean:write name="bt1" property="mach_Typ_Name"/>&nbsp;</td>
            <td class="TableItems2">
            <logic:notEmpty name="bt1" property="mach_Install_Date">
		<bean:define id="macInsDate1" name="bt1" property="mach_Install_Date"/><%= macInsDate1.toString().substring(0,10) %>
	   </logic:notEmpty>&nbsp;</td>
		<%
			}
		%> 
