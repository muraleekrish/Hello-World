...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.machine.MachineTypeListForm" />
...
<% 
	if (BuildConfig.DMODE)
		System.out.println("Machine Type List");
	...
	if(sortFieldName.equals("MC_TYP_NAME"))
	{
		columnNumber=1;
	}
	else if(sortFieldName.equals("MC_TYP_DESC"))
	{
		columnNumber=2;
	}
...
/* 	Creating the Home and Remote Objects 	*/
		...
		{  
		  ...
		  if (frm.getViewValidEntries().equalsIgnoreCase("yes"))
		  {
		  		...
		  		temp.setFieldName("MC_TYP_ISVALID");
		  ...
		  }else if (frm.getViewValidEntries().equalsIgnoreCase("no"))
		  {
			...
			if (!frm.getSearchText().equals(""))
			{
	   		...
	   		tempFilter.setFieldName("MC_TYP_NAME");
			   tempFilter.setFieldValue(frm.getSearchText());
			   String specialFunction="";
			   if(frm.getMachineType().equals("0"))
			   {
				   specialFunction = "Starts With";
			   }
			   else if(frm.getMachineType().equals("1"))
			   {
				   specialFunction = "Ends With";
			   }
			   else if(frm.getMachineType().equals("2"))
			   {
				   specialFunction = "Exactly";
			   }
			   else if(frm.getMachineType().equals("3"))
			   {
	   ...
	   int pagecount = Integer.parseInt(frm.getPage());
		...
		if (BuildConfig.DMODE)
 		...
 		HashMap machTypeDet = machObj.getAllMachineTypeDetails(objFilter, frm.getSortField(),order,((pageCount-1)*maxItems)+1,maxItems);
		
		Vector machTypeDetails = (Vector)machTypeDet.get("MachineTypeDetails");
		if (BuildConfig.DMODE)
			System.out.println("Machine Details Obj: "+machTypeDet);
		pageContext.setAttribute("machTypeDetails",machTypeDetails);
	  	totalCount = Integer.parseInt((machTypeDet.get("TotalRecordCount")).toString());
	  	if (BuildConfig.DMODE)
...
<script language="Javascript" type="text/Javascript">
	function viewMachineType(val)
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmMachineTypeEdit.do?formAction=view&id='+val;
	...
	function editItem()
	{
		if(modifyRow('machTypeList', document.forms[0]))
		{
		   var val = getCheckedValueDetail('machTypeList', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmMachineTypeEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	function makeInvalidItem()
	{
		if (validRow('machTypeList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmMachineTypeEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('machTypeList', document.forms[0]);
			var subQuery = '';
	...
	function makeValid()
	{
		var queryString = '<bean:message key="context"/>/frmMachineTypeEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('machTypeList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	function reLoadData()
	{
		...
		{
			<%
				String srchMachType = request.getParameter("searchText");
				session.setAttribute("searchText",srchMachType);
				String srchMachTypeCombo = request.getParameter("machineType");
				session.setAttribute("machineType",srchMachTypeCombo);
		...
		{
			if ('<%= request.getParameter("machTypeName") %>' != 'null')
				temp.searchText.value = '<%= request.getParameter("machTypeName") %>';
			else
				temp.searchText.value = "";

			if ('<%= request.getParameter("machTypeSrchTab") %>' != 'null')
				temp.machineType.value = '<%= request.getParameter("machTypeSrchTab") %>';
			else
			...
			if ('<%= request.getParameter("viewAll") %>' != 'null')
				...
				temp.viewValidEntries.value = "";
		}
	}
</script>
</head>

<body onLoad = "loadDefault(); sortByTable('machTypeList',<%=columnNumber%>,'<%=order%>'); reLoadData();">
<html:form action="frmMachineTypeList">
...
<table width="100%" height="100%" cellpadding="10" cellspacing="0">
  <tr>
    <td valign="top"><table width="100%" cellpadding="0" cellspacing="0" id="PageTitle">
        <tr>
            <td><bean:message key="prodacs.machine.machinetypelist.titleheader"/></td>
        </tr>
      </table>
        <br>
        ...
        <table width="100%" cellspacing="0" cellpadding="0" id="Filter">
          <tr> 
            <td><bean:message key="prodacs.machine.machinetype"/>
        ...
        <br>
        <table width="100%" cellpadding="0" cellspacing="0" id="machTypeList">
          <tr> 
            <td width="25" class="SortHeader"><input name="CheckAll" type="checkbox" id="CheckAll" value="checkbox" onClick="checkAll(document.MachineTypeList)"></td>
            <td width="200" class="SortHeader" onClick="sortTables(document.forms[0],this,'MC_TYP_NAME')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.machine.machinetype"/></td>
            <td class="SortHeader" onClick="sortTables(document.forms[0],this,'MC_TYP_DESC')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.employee.description"/></td>
          </tr>
          <logic:iterate id="bt1" name="machTypeDetails" indexId="count">
          <tr>
      <%
	      if(count.intValue()%2 == 0) 
   	   {
     	%>
				<logic:equal name="bt1" property="mc_Typ_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="mc_Typ_IsValid" value="1"><td  class="TableItems"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="mc_Typ_Id"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems"><a href="#" onMouseOver="window.status='View MachineType Info'; return true" onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="mc_Typ_Id"/>' onClick="viewMachineType(this.id)"><bean:write name="bt1" property="mc_Typ_Name"/>&nbsp;</a></td>
            <td class="TableItems"><bean:write name="bt1" property="mc_Typ_Desc"/>&nbsp;</td>
		<%
			} else
			{
		%>            
				<logic:equal name="bt1" property="mc_Typ_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="mc_Typ_IsValid" value="1"><td  class="TableItems2"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="mc_Typ_Id"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems2"><a href="#" onMouseOver="window.status='View MachineType Info'; return true" onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="mc_Typ_Id"/>' onClick="viewMachineType(this.id)"><bean:write name="bt1" property="mc_Typ_Name"/>&nbsp;</a></td>
            <td class="TableItems2"><bean:write name="bt1" property="mc_Typ_Desc"/>&nbsp;</td>
		<%
			}
		%> 
		   </tr>
