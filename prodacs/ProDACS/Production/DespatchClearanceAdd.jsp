...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.production.DespatchClearanceAddForm" />
...
<%
	...
	if (BuildConfig.DMODE)
		System.out.println("Despatch Clearance Add Form");
	...
	try
	{
		...
		{
			vecJobDet = objDesClear.getProdnJobDetailsByWorkOrder(Integer.parseInt(frm.getHidWrkOrdId()));
		...
		{
			vecJobOpnDet = objDesClear.getDespatchOperationDetails(Integer.parseInt(frm.getHidWrkOrdJobId()));
		...
		if (frm.getFormAction().equalsIgnoreCase("modify") && (request.getParameter("hidModDet") != null))
		{
			...
			vecJobDet = objDesClear.getProdnJobDetailsByWorkOrder(Integer.parseInt(arModDet[7].trim()));
			pageContext.setAttribute("vecJobDet",vecJobDet);

			vecJobOpnDet = objDesClear.getDespatchOperationDetails(Integer.parseInt(arModDet[8].trim()));
			pageContext.setAttribute("vecJobOpnDet",vecJobOpnDet);
		}
	}
	catch(Exception e)
	{
		if (BuildConfig.DMODE)
		{
			System.out.println("Problem in Despatch Clearance Add.jsp");
			e.printStackTrace();
		}
	}
%>
...
var oPopup;
	...
	function addItem()
	{
		document.forms[0].action = '<bean:message key="context"/>/Production/DespatchClearanceAdd.jsp?formAction=addNew';
		document.forms[0].submit();
	}
</script>
...
<html:form action="frmDespatchClearanceAdd">
