...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.production.DespatchClearanceForm" />
...
<%
	if (BuildConfig.DMODE)
		System.out.println("Despatch Clearance List");
	...
	Vector vec_DispatchClearance = new Vector();
	try
	{
		...
		if(sortFieldName.equals("DESPATCH_CRNT_DATE"))
			columnNumber = 1;
		...
		{  
			frm.setFormAction("");
		  	if (frm.getPostedView().equalsIgnoreCase("no"))
		  	{
		  		...
		  		temp.setFieldName("DESPATCH_POST_FLG");
  		...
  		else if(frm.getFormAction().equalsIgnoreCase("search"))
  		{
			...
			{
				Filter temp = new Filter();
				temp.setFieldName("DESPATCH_ISVALID");
				temp.setFieldValue("0");
				filter.add(temp);
			...
			if ((!frm.getProFromDate().equalsIgnoreCase("")) && (!frm.getProToDate().equalsIgnoreCase("")))
			{
				...
				Filter temp = new Filter();
				temp.setFieldName("DESPATCH_CRNT_DATE");
		...
		HashMap hmDispatchClearance = objDesClear.getAllDespatchClearanceDetails(objFilter, frm.getSortField(), order, ((pageCount-1)*maxItems)+1, maxItems);
		if (BuildConfig.DMODE)
			System.out.println("Dispatch Clearance (HM) :"+hmDispatchClearance);
		
		vec_DispatchClearance = (Vector)hmDispatchClearance.get("ProductionDetails");
		pageContext.setAttribute("vec_DispatchClearance",vec_DispatchClearance);
		
		/* For WorkOrder No */
		HashMap proWoNo = new HashMap();
		proWoNo = objDesClear.getWorkOrderList();
		pageContext.setAttribute("proWoNo",proWoNo);
		
		/* For Total Records Count */
	  	int totalCount = Integer.parseInt((hmDispatchClearance.get("TotalRecordCount")).toString()); 
		...
		if(totalPages==0)
			totalPages = 1;
		if (BuildConfig.DMODE)
			System.out.println("Despatch Clearance List Ends.");
	}
	catch(Exception e)
	{
		if (BuildConfig.DMODE)
		{
			System.out.println("Problem in DespatchClearanceList.jsp");
			e.printStackTrace();
		}
	}
%>
...
function Sort(tabName,columnIndex,ascending)
{
	 ...
	 else
	 {
	   ...
	   frmdespatchClearanceList.submit;
	 }
 }
var oPopup;
	function init()
...
<script language="Javascript" type="text/Javascript">
	...
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmdespatchClearanceEdit.do?formAction=view&id='+val;
	   document.forms[0].submit();
	}
	
	function editItem()
	{
		if(modifyRow('tbldespatchClearanceList', document.forms[0]))
		{
		   var val = getCheckedValueDetail('tbldespatchClearanceList', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmdespatchClearanceEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	...
	{
		if (validRow('tbldespatchClearanceList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmdespatchClearanceEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('tbldespatchClearanceList', document.forms[0]);
			var subQuery = '';
	...
	{
		var queryString = '<bean:message key="context"/>/frmdespatchClearanceEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('tbldespatchClearanceList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	{
		temp = document.forms[0];
		temp.action = '<bean:message key="context"/>/Production/DespatchClearanceAdd.jsp';
		temp.submit();
	}
...
<body onLoad="loadDefault(); init(); sortByTable('tbldespatchClearanceList',<%= columnNumber%>,'<%= order%>'); reLoadData();">
<html:form action="frmdespatchClearanceList">
      ...
      <td><table width="100%"  border="0" cellpadding="0" cellspacing="0" id="PageTitle">
        ...
        <table width="100"  border="0" cellspacing="0" cellpadding="0" align="right">
          <tr>
	 		...
	 		<menuconfig:userRights url="#" id="mkInValid" onMouseOver="window.status='Make Despatch Clearance Info InValid'; return true"  onMouseOut="window.status=''; return true" resourceId="1051" text="[ MakeInValid ]" classId="TopLnk" onClick="javaScript:makeInvalidItem();"/>
	 		<menuconfig:userRights url="#" id="mkValid" onMouseOver="window.status='Make Despatch Clearance Info Valid'; return true"  onMouseOut="window.status=''; return true" resourceId="1051" text="[ MakeValid ]" classId="TopLnk" style="display:none" onClick="javaScript:makeValidItem();"/>
          </tr>
        </table>
        <br>
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0" id="Filter">
          <tr>
            ...
            <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("proFromDate",DespatchClearanceList.proFromDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
            ...
            <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("proToDate",DespatchClearanceList.proToDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
		...
		<tr>
			<td>
			<html:messages id="messageid" message="true"><font class="message">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<bean:write name="messageid" /><br></html:messages></font>
			</td>
		</tr>
		</table>
		<table>
		...
		<br>
        <table width="100%" cellspacing="0" cellpadding="0" id="tbldespatchClearanceList">
        <tr>
            <td width="25" class="SortHeader"><input type="checkbox" name="CheckAll" value="checkbox" onClick="checkAll(document.DespatchClearanceList)"></td>
            <td width="100" class="SortHeader" onClick="sortTables(document.forms[0],this,'DESPATCH_CRNT_DATE')"><img src='<bean:message key="context"/>/images/sort_up.gif' width="7" height="8"><bean:message key="prodacs.job.date"/></td>
		...
		<logic:iterate id="bt1" name="vec_DispatchClearance" indexId="count">
		...
		{
		%>
		<logic:equal name="bt1" property="despatchIsValid" value="0"><td  class="InValid"></logic:equal>
		<logic:equal name="bt1" property="despatchIsValid" value="1"><td  class="TableItems"></logic:equal>	
		  <input name="CheckValue" type="checkbox" value='<bean:write name="bt1" property="despatchId"/>' onClick="isSimilar(this)"></td>
		  <td class="TableItems"><a href="#" onMouseOver="window.status='View Despatch Clearance Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="despatchId"/>' onClick="viewItem(this.id);"><bean:define id="despatchCrntDate" name="bt1" property="despatchCrntDate"/><%= despatchCrntDate.toString().substring(0,10) %>&nbsp;</a></td>
		...
		<logic:equal name="bt1" property="despatchIsValid" value="1"><td  class="TableItems"></logic:equal>	
		  <input name="CheckValue" type="checkbox" value='<bean:write name="bt1" property="despatchId"/>' onClick="isSimilar(this)"></td>
		  <td class="TableItems2"><a href="#" onMouseOver="window.status='View Despatch Clearance Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="despatchId"/>' onClick="viewItem(this.id);"><bean:define id="despatchCrntDate1" name="bt1" property="despatchCrntDate"/><%= despatchCrntDate1.toString().substring(0,10) %>&nbsp;</a></td>
