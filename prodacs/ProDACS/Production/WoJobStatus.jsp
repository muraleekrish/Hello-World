...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.production.WOJobStatusListForm" />
...
<useradmin:userrights resource="55"/>
<%
   ...
   GregorianCalendar proTDate = new GregorianCalendar();
	...
	try
	{
		...
		/* By Customer Type */
		HashMap custTypes = productionObj.getCustomerTypes();
		pageContext.setAttribute("custTypes",custTypes);
		
		/* Loading Customer Names via Customer Type */
		HashMap custNames = new HashMap();
		if (!(frm.getCustType().equalsIgnoreCase("0")))
		{
			custNames = productionObj.getCustomerNameByType(Integer.parseInt(frm.getCustType()));
		...
		Vector vec_JobStatusDetails = new Vector();
		hmjobStatus = productionObj.getAllJobStatusDetails(Integer.parseInt(frm.getCustName()), proFDate.getTime(), proTDate.getTime(), objFilter, sortFieldName, order, ((pageCount-1)*maxItems)+1, maxItems);
	
		vec_JobStatusDetails = (Vector)hmjobStatus.get("JobStatusDetails");
...
<script language="Javascript" type="text/Javascript">
	...
	function navigation()
	{
		...
		if ((temp.custType.options[temp.custType.selectedIndex].value == "0") ||
			(temp.custName.options[temp.custName.selectedIndex].value == "0"))
		{
			alert("Both Customer Type and Name are Mandatory!");
			return false;
		}
	...
	function loadCustNames()
	{
		temp = document.forms[0];
		if (temp.custType.options[temp.custType.selectedIndex].value != "0")
		{
			temp.formAction.value = "getCustNames";
			temp.submit();
		}
	}
</script>
...
<html:form action="frmWOJobStatusList">
		...
		<table width="100%" cellspacing="0" cellpadding="0" id="Filter" border="0">
          	<tr> 
      	      ...
      	      <td width="100"><html:text property="proFromDate" styleClass="TextBox" size="12" readonly="true"/> 
            	  <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar_previous("proFromDate",WOJobStatusList.proFromDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
			...
			<tr>
				<td width="100"><bean:message key="prodacs.customer.customertype"/><span class="mandatory">*</span></td>
				<td>:</td>
				<td>
					<html:select property="custType" styleClass="Combo" onchange="loadCustNames();">
      	      	    	<html:option value="0">-- Choose Customer Type --</html:option>
						<html:options collection="custTypes" property="key" labelProperty="value"/>
	                </html:select>
				</td>
				<td width="100"><bean:message key="prodacs.customer.customername"/><span class="mandatory">*</span></td>
				<td>:</td>
				<td>
					<html:select property="custName" styleClass="Combo">
      	      	    	<html:option value="0">-- Choose Customer Name --</html:option>
