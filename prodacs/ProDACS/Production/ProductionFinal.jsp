...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.production.ProductionFinalForm" />
...
<%
	if (BuildConfig.DMODE)
		System.out.println("Production Final List");
	...
	{
		if (BuildConfig.DMODE)
			System.out.println("Production Final List Starts.");
		...
		if(sortFieldName.equals("FPROD_CRNT_DATE"))
		{
			columnNumber = 1;
		}
		...
		{  
			frm.setFormAction("");
		  	if (frm.getPostedView().equalsIgnoreCase("no"))
		  	{
		  		...
		  		temp.setFieldName("FPROD_POST_FLG");
  		...
  		else if(frm.getFormAction().equalsIgnoreCase("search"))
  		{
			  ...
			  {
			  		Filter temp = new Filter();
			  		temp.setFieldName("FPROD_ISVALID");
			  		temp.setFieldValue("0");
			  		filter.add(temp);
			...
			if ((!frm.getProFromDate().equalsIgnoreCase("")) && (!frm.getProToDate().equalsIgnoreCase("")))
			{
				...
				Filter temp = new Filter();
				temp.setFieldName("FPROD_CRNT_DATE");
		...
		HashMap prodFinalDets = objProdFinal.getAllFinalProductionDetails(objFilter, frm.getSortField(), order, ((pageCount-1)*maxItems)+1, maxItems);
		if (BuildConfig.DMODE)
			System.out.println("Production Final (HM) :"+prodFinalDets);
		
		vec_ProFinalDets = (Vector)prodFinalDets.get("ProductionDetails");
		pageContext.setAttribute("vec_ProFinalDets",vec_ProFinalDets);
		
		/* For WorkOrder No */
		HashMap proWoNo = new HashMap();
		proWoNo = objProdFinal.getWorkOrderList();
		pageContext.setAttribute("proWoNo",proWoNo);
		
	  	int totalCount = Integer.parseInt((prodFinalDets.get("TotalRecordCount")).toString()); 
		...
		if (BuildConfig.DMODE)
			System.out.println("ProductionFinal List Ends.");
	}
	catch(Exception e)
	{
		if (BuildConfig.DMODE)
		{
			System.out.println("Problem in ProductionFinal.jsp");
			e.printStackTrace();
		}
	}
%>
...
<script language="Javascript" type="text/Javascript">
	...
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmProdFinalEdit.do?formAction=view&id='+val;
	   document.forms[0].submit();
	}
	
	function editItem()
	{
		if(modifyRow('prodFinalList', document.forms[0]))
		{
		   var val = getCheckedValueDetail('prodFinalList', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmProdFinalEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	...
	{
		if (validRow('prodFinalList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmProdFinalEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('prodFinalList', document.forms[0]);
			var subQuery = '';
	...
	{
		var queryString = '<bean:message key="context"/>/frmProdFinalEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('prodFinalList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	function reLoadData()
	{
		...
		else if (temp.formAction.value == "afterView") 
		{
			...
			if ('<%= request.getParameter("postedView") %>' != 'null')
				...
				temp.postedView.value = "0";

			if ('<%= request.getParameter("viewAll") %>' != 'null')
				temp.listValidEntries.value = '<%= request.getParameter("viewAll") %>';
			else
				temp.listValidEntries.value = "0";
		}
	}
</script>
</head>
<body onLoad="loadDefault(); init(); sortByTable('prodFinalList',<%= columnNumber%>,'<%= order%>'); reLoadData();">
<html:form action="frmProdFinalList">
      ...
      <td><table width="100%"  border="0" cellpadding="0" cellspacing="0" id="PageTitle">
        ...
        <table width="100"  border="0" cellspacing="0" cellpadding="0" align="right">
          <tr>
	 		...
	 		<menuconfig:userRights url="#" id="#" onMouseOver="window.status='Modify Production Final Info'; return true"  onMouseOut="window.status=''; return true" resourceId="1049" text="[ Modify ]" classId="TopLnk" onClick="javaScript:editItem();"/>
	 		<menuconfig:userRights url="#" id="mkInValid" onMouseOver="window.status='Make Production Final Info InValid'; return true"  onMouseOut="window.status=''; return true" resourceId="1049" text="[ MakeInValid ]" classId="TopLnk" onClick="javaScript:makeInvalidItem();"/>
	 		<menuconfig:userRights url="#" id="mkValid" onMouseOver="window.status='Make Production Final Info Valid'; return true"  onMouseOut="window.status=''; return true" resourceId="1049" text="[ MakeValid ]" classId="TopLnk" style="display:none" onClick="javaScript:makeValidItem();"/>
          </tr>
        </table>
        <br>
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0" id="Filter">
          <tr>
            ...
            <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("proFromDate",ProductionFinalList.proFromDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
            ...
            <img src='<bean:message key="context"/>/images/calendar.gif' width="16" height="15" align="absmiddle" onClick='show_calendar("proToDate",ProductionFinalList.proToDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
		...
		<br>
        <table width="100%" cellspacing="0" cellpadding="0" id="prodFinalList">
        <tr>
            ...
            <td width="100" class="SortHeader" onClick="sortTables(document.forms[0],this,'FPROD_CRNT_DATE')"><img src='<bean:message key="context"/>/images/sort_up.gif' width="7" height="8"><bean:message key="prodacs.job.date"/></td>
		...
		{
		%>
		<logic:equal name="bt1" property="finalProdIsValid" value="0"><td  class="InValid"></logic:equal>
		<logic:equal name="bt1" property="finalProdIsValid" value="1"><td  class="TableItems"></logic:equal>	
		  <input name="CheckValue" type="checkbox" value='<bean:write name="bt1" property="finalProdId"/>' onClick="isSimilar(this)"></td>
		  <td class="TableItems"><a href="#" onMouseOver="window.status='View Production Final Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="finalProdId"/>' onClick="viewItem(this.id);"><bean:define id="finalCrntDate" name="bt1" property="finalCrntDate"/><%= finalCrntDate.toString().substring(0,10) %>&nbsp;</a></td>
		...
		<logic:equal name="bt1" property="finalProdIsValid" value="1"><td  class="TableItems"></logic:equal>	
		  <input name="CheckValue" type="checkbox" value='<bean:write name="bt1" property="finalProdId"/>' onClick="isSimilar(this)"></td>
		  <td class="TableItems2"><a href="#" onMouseOver="window.status='View Production Final Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="finalProdId"/>' onClick="viewItem(this.id);"><bean:define id="finalCrntDate1" name="bt1" property="finalCrntDate"/><%= finalCrntDate1.toString().substring(0,10) %>&nbsp;</a></td>
