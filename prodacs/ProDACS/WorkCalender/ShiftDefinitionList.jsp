...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.workcalendar.ShiftDefinitionListForm" />
...
<% 
	...
	if(sortFieldName.equals("SHIFT_NAME"))
	{
		columnNumber=1;
	}
	else if(sortFieldName.equals("SHIFT_DESC"))
	{
		columnNumber=2;
	}
...
/* 	Creating the Home and Remote Objects 	*/
		...
		{  
		  ...
		  if (frm.getListValidEntries().equalsIgnoreCase("yes"))
		  {
		  		...
		  		temp.setFieldName("SHIFT_ISVALID");
	   ...
	   int pagecount = Integer.parseInt(frm.getPage());
 		...
 		HashMap shiftDefDetails = shiftDefObj.getAllShiftDefnDetails(objFilter, frm.getSortField(), order, ((pageCount-1)*maxItems)+1, maxItems);
		if(BuildConfig.DMODE)
			System.out.println("ShiftDefinitionDetails(HM): "+shiftDefDetails);
		
		Vector vec_shiftDef = (Vector) shiftDefDetails.get("ShiftDefnDetails");
		
	 	pageContext.setAttribute("vec_shiftDef",vec_shiftDef); 
	 	
	  	int totalCount = Integer.parseInt((shiftDefDetails.get("TotalRecordCount")).toString()); 
...
<html:html>
<head>
<title><bean:message key="prodacs.workcalendar.shiftdefinition.header"/></title>
...
<script language="Javascript" type="text/Javascript">
	...
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmShiftDftnEdit.do?formAction=view&id='+val;
	   document.forms[0].submit();
	}
	
	function editItem()
	{
		if(modifyRow('tblShiftDftn', document.forms[0]))
		{
		   var val = getCheckedValueDetail('tblShiftDftn', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmShiftDftnEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	...
	{
		temp = document.forms[0];
		var obj = document.getElementById('tblShiftDftn');

		if (deleteRow('tblShiftDftn', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmShiftDftnEdit.do?formAction=delete';
			var val = getMultipleCheckedValue('tblShiftDftn', document.forms[0]);
			var subQuery = '';
	...
	{
		if (validRow('tblShiftDftn', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmShiftDftnEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('tblShiftDftn', document.forms[0]);
			var subQuery = '';
			for(var i=0;i<val.length;i++)
			{
	...
	{
		var queryString = '<bean:message key="context"/>/frmShiftDftnEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('tblShiftDftn', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	{
		temp = document.forms[0];
		temp.action = '<bean:message key = "context" />/WorkCalender/ShiftDefinitionAdd.jsp';
		temp.submit();
	}
...
<body onLoad="loadDefault();sortByTable('tblShiftDftn',<%= columnNumber%>,'<%= order%>'); reLoadData();">
<html:form action="frmShiftDftnList">
  ...
  <table width="100%" height="100%" cellpadding="10" cellspacing="0">
    <tr>
      <td valign="top"><table width="100%" cellpadding="0" cellspacing="0" id="PageTitle">
        ...
        <br>
        <table width="100%" cellpadding="0" cellspacing="0" id="tblShiftDftn">
          <tr> 
            <td width="25" class="SortHeader"><input name="CheckAll" type="checkbox" id="CheckAll" value="checkbox" onClick="checkAll(document.ShiftDefinitionList)"></td>
            <td width="200" class="SortHeader" onClick="sortTables(document.forms[0],this,'SHIFT_NAME')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.workcalendar.shiftdefinition.shiftname"/></td>
            <td class="SortHeader" onClick="sortTables(document.forms[0],this,'SHIFT_DESC')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.workcalendar.shiftdefinition.shiftdescription"/></td>
          </tr>
          <logic:iterate id="bt1" name="vec_shiftDef" indexId="count">
          <tr>
      <%
	      if(count.intValue()%2 == 0) 
   	   {
     	%>
				<logic:equal name="bt1" property="shiftIsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="shiftIsValid" value="1"><td  class="TableItems"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="shiftId"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems"><a href="#" onMouseOver="window.status='View Shift Info'; return true" onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="shiftId"/>' onClick="viewItem(this.id);"><bean:write name="bt1" property="shiftName"/>&nbsp;</a></td>
            <td class="TableItems"><bean:write name="bt1" property="shiftDesc"/>&nbsp;</td>
		<%
			} else
			{
		%>            
				<logic:equal name="bt1" property="shiftIsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="shiftIsValid" value="1"><td  class="TableItems2"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="shiftId"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems2"><a href="#" onMouseOver="window.status='View Shift Info'; return true" onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="shiftId"/>' onClick="viewItem(this.id);"><bean:write name="bt1" property="shiftName"/>&nbsp;</a></td>
            <td class="TableItems2"><bean:write name="bt1" property="shiftDesc"/>&nbsp;</td>
		<%
			}
		%> 
		   </tr>
