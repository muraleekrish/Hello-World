...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.workcalendar.AvailCalendarAddForm" />
<jsp:setProperty name="frm" property="*" /> 

<useradmin:userrights resource="1010"/>
<%
	if(BuildConfig.DMODE)
		System.out.println("Availability Add");
...
<script language="Javascript" type="text/Javascript"> 
		...
		if (temp.bcStarnEndDay.value != "")
		{
			...
			for (var i = 0; i < obj.children(0).children(0).children.length; i++)
			{
				...
				if (baseCalDays.indexOf("5") != -1)
					...
					obj.children(0).children(0).children(i).children(4).className = 'spl';

				if (baseCalDays.indexOf("6") != -1)
					obj.children(0).children(0).children(i).children(5).className = 'test';
				else
					obj.children(0).children(0).children(i).children(5).className = 'spl';
	...
	function loadCrntDate()
	{
		...
		{
			tblMandatory.style.display = 'block';
			avlCalendar.style.display = 'none';
...
<script language="Javascript" type="text/Javascript">
	...
	{
		var temp = document.forms[0];
		var stDt = (temp.fromDate.value).split("-");
		var edDt = (temp.toDate.value).split("-");
		var stDate = new Date((parseInt(stDt[0],10)),(parseInt(stDt[1],10)-1),(parseInt(stDt[2],10)));
		var edDate = new Date((parseInt(edDt[0],10)),(parseInt(edDt[1],10)-1),(parseInt(edDt[2],10)));
		if (stDate > edDate)
		{
			alert("Start Date cannot be greater than End Date!");
			return false;
		}
		temp.formAction.value = "loadMandatory";
		temp.submit();
	}

	/* Load the Availabilty and Non-availability Details from the Calendar */
	function loadAvlnNonAvl()
	{
		var temp = document.forms[0];
		...
		{
			for (var i = 0; obj.children(0).children.length > 1; i++)
			{
		...
		else if (obj.children(0).children.length == 1)
		{
				obj.children(0).children(0).children(0).children(0).checked = false;
				obj.children(0).children(0).children(1).children(0).value = "";
				obj.children(0).children(0).children(2).children(0).value = "";
		...
		for (var i = 0; i < selDates.length; i++)
		{
			...
			arSftDetails = (temp.shiftDetails.value).split('^');
			var tblSftDetails = "";
			var cnt = 0;
			...
			//alert (tblSftDetails);
			//alert (bigDay[crntDate.getDay()]);
			//alert (baseCalDays.indexOf(parseInt(crntDate.getDay(),10)));
			...
			if ((obj.children(0).children(i).children(0).children(0).checked) && (obj.children(0).children(i).children(1).children(0).value != ""))
			{
				...
				else
				{
					...
					if (obj.children(0).children(i).children(3).children(0).value != "Non-Availability")
					{
						...
						for (var k = 0; k < arSftDetail.length; k++)
						{
							arPrnlSftDetail = arSftDetail[k].split('-');
							 
							if ((obj1.children.length == 1) && (obj1.children(0).children(0).children(1).children(0).value == ""))
							{
								obj1.children(0).children(0).children(1).children(0).value = bigDay[parseInt(arPrnlSftDetail[0],10)-1];
								...
								obj1.children(0).children(0).children(5).children(0).value = arPrnlSftDetail[6];
								obj1.children(0).children(0).children(6).children(0).value = arPrnlSftDetail[0];
								obj1.children(0).children(0).children(7).children(0).value = arPrnlSftDetail[7];
								obj1.children(0).children(0).children(8).children(0).value = arPrnlSftDetail[8];
							}
							else
							{
								...
								obj1.children(len-1).children(0).children(1).children(0).value = bigDay[parseInt(arPrnlSftDetail[0],10)-1];
								...
								obj1.children(len-1).children(0).children(5).children(0).value = arPrnlSftDetail[6];
								obj1.children(len-1).children(0).children(6).children(0).value = arPrnlSftDetail[0];
								obj1.children(len-1).children(0).children(7).children(0).value = arPrnlSftDetail[7];
								obj1.children(len-1).children(0).children(8).children(0).value = arPrnlSftDetail[8];
							}
						}
						...
						temp.dayofWeek[1] = new Option(bigDay[parseInt(arPrnlSftDetail[0],10)-1],arPrnlSftDetail[0]);
					}
					else // Making Non-Availability to Custom Availability
					{
						...
						if (obj.children(0).children(i).children(4).children(0).value == "")
						{
							...
							var arSftDetails = (temp.shiftDetails.value).split('^'); // Calculation of Previous
							var tblSftDetails = "";									 // valid Day Last Sft Time
							for (var k = 0; k < arSftDetails.length; k++)
							{
								arPrnlSftDetails = arSftDetails[k].split('-');
							...
							var arSftDetails = (temp.shiftDetails.value).split('^'); // Calculation of Next
							var tblSftDetails = "";									 // valid Day First Sft Time
							var cnt = 0;
							for (var k = 0; k < arSftDetails.length; k++)
							{
								arPrnlSftDetails = arSftDetails[k].split('-');
							...
							temp.shiftName.value = "0";
							temp.startHours.value = "00";
							temp.startMins.value = "00";
							temp.startTimeDay.value = "0";
							temp.endHours.value = "00";
							temp.endMins.value = "00";
							temp.endTimeDay.value = "0";
	...
	/* Modification for shift details */
	function modRow(formObj)
	{
		...
		for (var i = 0; i < obj.children.length; i++) // Placing values to appropriate positions
		{
			if (obj.children(i).children(0).children(0).children(0).checked)
			{
				temp.dayofWeek.selectedIndex = 1;//parseInt(obj.children(i).children(0).children(6).children(0).value,10)-1;
				temp.shiftName.selectedIndex = obj.children(i).children(0).children(7).children(0).value;
				loadTimeDate();
	...
	function addShift(formObj)
	{
		...
		if (nonAvlFlag) // only for Non-ability to Custom Availability
		{
			...
			pEdDay = pStTime[1];
			//alert(pStDay+" == "+stSDay+" == "+pStTime+" > "+sTime);
			if (pStDay == stSDay)
			{
				if (pStTime > sTime)
				{
	...
	function moveToBuffer()
	{
		...
		for (var i = 0; i < obj.children.length; i++)
		{
			...
			bffrSftDetails = bffrSftDetails + obj.children(i).children(0).children(6).children(0).value + "-" +
	...
	function createAvailability()
	{
		...
		for (var i = 0; i < obj.children(0).children.length; i++)
		{
			...
			temp.avlDetails.value = temp.avlDetails.value + obj.children(0).children(i).children(1).children(0).value + '#' +
				obj.children(0).children(i).children(2).children(0).value + '#' +
...
<body onload="init(); loadCrntDate();">
<html:form action="frmAvailClndrAdd" focus="baseCalendarName">
<html:hidden property="formAction"/>
<html:hidden property="shiftDetails"/>
...
<td>
        ...
        <tr> 
			...
			<td>
				...
				<tr> 
			   <td width="110" class="FieldTitle"><bean:message key="prodacs.workcalendar.availabilitycalendar.calendarname"/><span class="Mandatory">*</span></td>
				<td width="1" class="FieldTitle">:</td>
				<td width="100" class="FieldTitle"><html:text property="baseCalendarName" styleClass="TextBox" size="30"/></td>
				<td width="430">&nbsp;</td>
				</tr>
				<tr> 
				<td class="FieldTitle"><bean:message key="prodacs.reports.startdate"/><span class="Mandatory">*</span></td>
				<td class="FieldTitle">:</td>
				<td width="150" class="FieldTitle"><html:text property="fromDate" styleClass="TextBox" readonly="true" size="11"  /> <img src='<bean:message key="context"/>/images/calendar.gif' name='calImg' align='absmiddle' onclick='show_calendar("fromDate",AvailCalendarAdd.fromDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
				<td>&nbsp;</td>
			 </tr>
			 <tr>     
				<td class="FieldTitle"><bean:message key="prodacs.reports.enddate"/><span class="Mandatory">*</span></td>
				<td class="FieldTitle">:</td>
				<td width="150" class="FieldTitle"><html:text property="toDate" styleClass="TextBox" size="11" readonly="true" /> <img src='<bean:message key="context"/>/images/calendar.gif' name='calImg' align='absmiddle' onclick='show_calendar("toDate",AvailCalendarAdd.toDate.value,event.clientX,event.clientY,false)' onMouseOver="this.style.cursor='hand'"></td>
				<td>&nbsp;</td>
			 </tr>
			 <tr> 
				<td class="FieldTitle"><bean:message key="prodacs.workcalendar.basecalendar.header"/><span class="Mandatory">*</span></td>
				<td class="FieldTitle">:</td>
				<td colspan="4" class="FieldTitle">
					<html:select property="selectBaseCalendar" styleClass="Combo">
			...
			<table width="100%" cellpadding="0" cellspacing="0" id="BtnBg">
			 <tr>
			  <td><html:button property="availDet" styleClass="Button" onclick="loadAvailCal();"> Next </html:button></td>
			 </tr>
			</table>
		</tr>
		</td>
		...
		<table width="100%" cellspacing="0" cellpadding="10" border="0" id="avlCalendarNext">
		<tr>
		<td>
			<table width="100%" cellpadding="0" cellspacing="0" id="BtnBg">
			 <tr>
			  <td><html:button property="availDet" styleClass="Button" onclick="loadAvlnNonAvl();"> Next </html:button></td>
			 </tr>
			</table>
		</td>
		</tr>
	...
	<td>
		<table width="90%" cellpadding="0" cellspacing="0" id="BtnBg">
		 <tr>
		  <td><html:button property="addAvailCalendar" styleClass="Button" onclick="createAvailability();">Add Availability</html:button></td>
		 </tr>
		</table>
	</td>
	</tr>
