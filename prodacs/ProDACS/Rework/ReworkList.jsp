...
<jsp:useBean id="frm" class="com.savantit.prodacs.presentation.tableadmin.rework.ReworkListForm" />
...
<% 
	if (BuildConfig.DMODE)
		System.out.println("Rework List");
...
/* 	Creating the Home and Remote Objects 	*/
		...
		{  
		  ...
		  }else if (frm.getValidEntry().equalsIgnoreCase("no"))
		  {
			...
			/* Sort by Rework Category */
			if (!frm.getReworkCategory().equalsIgnoreCase("0"))
			{
				Filter temp = new Filter();
				temp.setFieldName("RWK_CATEGORY_ID");
				temp.setFieldValue(frm.getReworkCategory());
				filter.add(temp);
			}
	   ...
	   int pagecount = Integer.parseInt(frm.getPage());
 		...
 		HashMap reworkDetails = reworkObj.getAllReworkDetails(objFilter, frm.getSortField(), order, ((pageCount-1)*maxItems)+1, maxItems);
		if (BuildConfig.DMODE)
			System.out.println("reworkDetails(HM): "+reworkDetails);
			
		/* Hashmap for loading ReworkCategory */
		HashMap reworkCategory = reworkObj.getAllReworkCategories();
		if (BuildConfig.DMODE)
			System.out.println("reworkCategory(HM): "+reworkCategory);
			
	 	Vector reworkDet = (Vector)reworkDetails.get("ReworkDetails");

	 	pageContext.setAttribute("reworkDet",reworkDet); 
	 	pageContext.setAttribute("reworkCategory",reworkCategory);
	 	
	  	int totalCount = Integer.parseInt((reworkDetails.get("TotalRecordCount")).toString()); 
...
<script language="Javascript" type="text/Javascript">
	...
	{
	   document.forms[0].action = '<bean:message key="context"/>/frmReworkEdit.do?formAction=view&id='+val;
	   document.forms[0].submit();
	}
	
	function editItem()
	{
		if(modifyRow('reWrkList', document.forms[0]))
		{
		   var val = getCheckedValueDetail('reWrkList', document.forms[0]);
		   document.forms[0].action = '<bean:message key="context"/>/frmReworkEdit.do?formAction=modify&id='+val;
		   document.forms[0].submit();
		}
	}
	...
	{
		if (validRow('reWrkList', document.forms[0]))
		{
			var queryString = '<bean:message key="context"/>/frmReworkEdit.do?formAction=makeInvalid';
			var val = getMultipleCheckedValue('reWrkList', document.forms[0]);
			var subQuery = '';
	...
	{
		var queryString = '<bean:message key="context"/>/frmReworkEdit.do?formAction=makeValid';
		var val = getMultipleCheckedValue('reWrkList', document.forms[0]);
		var subQuery = '';
		for(var i=0;i<val.length;i++)
		{
	...
	{
		temp = document.forms[0];
		temp.action = '<bean:message key="context"/>/Rework/ReworkAdd.jsp';
		temp.submit();
	}
	...
	function reLoadData()
	{
		...
		{
			<%
				String rwkCategory = request.getParameter("reworkCategory");
				session.setAttribute("reworkCategory",rwkCategory);
		...
		{
			if ('<%= request.getParameter("rwkCategory") %>' != 'null')
				temp.reworkCategory.value = '<%= request.getParameter("rwkCategory") %>';
			else
				temp.reworkCategory.value = "";

			if ('<%= request.getParameter("viewAll") %>' != 'null')
				temp.validEntry.value = '<%= request.getParameter("viewAll") %>';
			else
				temp.validEntry.value = "0";
		}
	}
...
<body onLoad="loadDefault();sortByTable('reWrkList',<%= columnNumber%>,'<%= order%>'); reLoadData();">
<html:form action="frmReworkList">
    ...
    <tr>
      <td><table width="100%" cellpadding="0" cellspacing="0" id="PageTitle">
        ...
        <tr> 
 		...
 		<menuconfig:userRights url="#" id="mkValid" onMouseOver="window.status='Make Rework Info Valid'; return true"  onMouseOut="window.status=''; return true" resourceId="1012" text="[ MakeValid ]" classId="TopLnk" style="display:none" onClick="javaScript:makeValidItem();"/>
        </tr>
        </table>
        ...
        <table width="100%" cellspacing="0" cellpadding="0" id="Filter">
          <tr> 
            <td><bean:message key="prodacs.rework.reworkcategory"/>
              <html:select property="reworkCategory" styleClass="Combo">
        ...
        <br>
        <table width="100%" cellpadding="0" cellspacing="0" id="reWrkList">
          <tr> 
            <td width="25" class="SortHeader"><input name="CheckAll" type="checkbox" id="CheckAll" value="checkbox" onClick="checkAll(document.ReworkList)"></td>
            <td width="250" class="SortHeader" onClick="sortTables(document.forms[0],this,'RWK_CATEGORY')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.rework.reworkcategory"/></td>
            <td class="SortHeader" onClick="sortTables(document.forms[0],this,'RWK_RSN')"><img src='<bean:message key="context"/>/images/sort.gif' width="7" height="8"> 
              <bean:message key="prodacs.rework.reworkreason"/></td>
          </tr>
       <logic:iterate id="bt1" name="reworkDet" indexId="count">
          <tr>
      <%
	      if(count.intValue()%2 == 0) 
   	   {
     	%>
				<logic:equal name="bt1" property="rwk_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="rwk_IsValid" value="1"><td  class="TableItems"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="rwk_Id"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems"><a href="#" onMouseOver="window.status='View Rework Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="rwk_Id"/>' onClick="viewItem(this.id)"><bean:write name="bt1" property="rwk_Category"/>&nbsp;</a></td>
            <td class="TableItems"><bean:write name="bt1" property="rwk_Rsn"/>&nbsp;</td>
		<%
			} else
			{
		%>            
				<logic:equal name="bt1" property="rwk_IsValid" value="0"><td  class="InValid"></logic:equal>
	         <logic:equal name="bt1" property="rwk_IsValid" value="1"><td  class="TableItems2"></logic:equal>	
	         
            <input name="CheckValue" type="checkbox" id="CheckValue" value='<bean:write name="bt1" property="rwk_Id"/>' onClick="isSimilar(this)"></td>
            <td class="TableItems2"><a href="#" onMouseOver="window.status='View Rework Info'; return true"  onMouseOut="window.status=''; return true" id='<bean:write name="bt1" property="rwk_Id"/>' onClick="viewItem(this.id)"><bean:write name="bt1" property="rwk_Category"/>&nbsp;</a></td>
            <td class="TableItems2"><bean:write name="bt1" property="rwk_Rsn"/>&nbsp;</td>
		<%
			}
		%> 
		   </tr>
